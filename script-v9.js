...
// V9 lightweight motion fallback for mobile and all major browsers.
document.documentElement.classList.add('motion-lite');
const v9Items=[...document.querySelectorAll('.reveal')];
const v9Show=(el)=>{if(el&&!el.classList.contains('is-visible'))el.classList.add('is-visible');};
if('IntersectionObserver' in window){
  const v9Observer=new IntersectionObserver((entries)=>{
    entries.forEach((entry)=>{if(entry.isIntersecting){v9Show(entry.target);v9Observer.unobserve(entry.target);}});
  },{threshold:0.02,rootMargin:'0px 0px -1% 0px'});
  v9Items.forEach((el)=>v9Observer.observe(el));
}else{v9Items.forEach(v9Show);}
setTimeout(()=>{v9Items.forEach((el)=>{if(el.getBoundingClientRect().top<innerHeight*1.6)v9Show(el);});},650);
addEventListener('scroll',()=>{v9Items.forEach((el)=>{if(!el.classList.contains('is-visible')&&el.getBoundingClientRect().top<innerHeight*1.15)v9Show(el);});},{passive:true});
addEventListener('pageshow',()=>{document.documentElement.classList.add('motion-lite');v9Items.forEach((el)=>{if(el.getBoundingClientRect().top<innerHeight*1.25)v9Show(el);});});
